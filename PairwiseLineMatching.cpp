...
double PairwiseLineMatching::GlobalRotationOfImagePair_(ScaleLines &linesInLeft, ScaleLines &linesInRight)
{
    ...
    VectorXd angleHistLeft(dim);
    VectorXd angleHistRight(dim);
    VectorXd lengthLeft(dim);//lengthLeft[i] store the total line length of all the lines in the ith angle bin.
    VectorXd lengthRight(dim);
    ...
    VectorXd difVec(dim);//the difference vector between left histogram and shifted right histogram
    ...
    VectorXd lengthDifVec(dim);//the length difference vector between left and right
...
void PairwiseLineMatching::BuildAdjacencyMatrix_(ScaleLines &linesInLeft,ScaleLines &linesInRight)
{
    ...
    MatrixXd desDisMat(numLineLeft,numLineRight);//store the descriptor distance of lines in left and right images.
    ...
    //	testMat.SetZero();
    VectorXd adjacenceVec(dim*(dim+1)/2);
     ...
     */
    Matrix<unsigned int, Dynamic, Dynamic> bComputedLeft(numLineLeft,numLineLeft);//flag to show whether the ith pair of left image has already been computed.
    bComputedLeft.setZero();
    MatrixXd intersecRatioLeft(numLineLeft,numLineLeft);//the ratio of intersection point and the line in the left pair
    MatrixXd projRatioLeft(numLineLeft,numLineLeft);//the point to line distance divided by the projected length of line in the left pair.

    Matrix<unsigned int, Dynamic, Dynamic> bComputedRight(numLineRight,numLineRight);//flag to show whether the ith pair of right image has already been computed.
    bComputedRight.setZero();
    MatrixXd       intersecRatioRight(numLineRight,numLineRight);//the ratio of intersection point and the line in the right pair
    MatrixXd       projRatioRight(numLineRight,numLineRight);//the point to line distance divided by the projected length of line in the right pair.
                                                                   ...
                                                                   std::vector<unsigned int > &matchResult)
{
    ...
    std::fstream eigenMap;
    eigenMap.open("eigenMap.txt", std::ios::out);

    MatrixXd mat(linesInLeft.size(),linesInRight.size());
    mat.setZero();
